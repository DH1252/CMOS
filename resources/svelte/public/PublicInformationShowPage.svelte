...
<article class="space-y-8">
  ...
  <header class="max-w-[70ch] space-y-4 border-b border-[var(--landing-terminal-line-resolved)] pb-6">
    <TerminalTextReveal animate={false} tag="h1" text={article.title} textClass="text-4xl leading-tight text-[var(--landing-terminal-heading-resolved)] md:text-5xl" />
  ...
  <div class="grid gap-8 lg:grid-cols-[minmax(0,1.12fr)_18rem] lg:items-start">
    ...
    <aside class="grid gap-6 lg:sticky lg:top-24">
      ...
      <div class="landing-panel px-5 py-5">
        <h2 class="text-lg text-[var(--landing-terminal-heading-resolved)]">Artikel lainnya</h2>

        {#if !latestArticles.length}
          <p class="mt-3 text-sm leading-7 text-[var(--landing-terminal-soft-resolved)]">Belum ada artikel lain.</p>
        {:else}
