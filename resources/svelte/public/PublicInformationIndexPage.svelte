...
<section class="space-y-10">
  ...
  {#if featured}
    <section class="grid gap-6 border-b border-border pb-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-start">
      ...
      {:else}
        <div class="flex items-center justify-center rounded-[10px] border border-border bg-card px-6 py-16 text-sm text-muted-foreground">Tanpa sampul</div>
      {/if}
      ...
      {#each articles as article (article.href)}
        <a href={article.href} class="grid gap-4 border-t border-border pt-5 text-inherit no-underline transition-colors hover:text-brand-secondary md:grid-cols-[12rem_minmax(0,1fr)] first:border-t-0 first:pt-0">
          {#if article.coverImage}
            <OptimizedImage src={article.coverImage} alt={article.title} class="w-full rounded-[10px] border border-border" loading="lazy" decoding="async" sizes="12rem" onerror={handleImageError} />
          {:else}
            <div class="flex items-center justify-center rounded-[10px] border border-border bg-card px-5 py-12 text-sm text-muted-foreground">
              Tanpa sampul
            </div>
          {/if}
