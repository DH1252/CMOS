...
<Empty.Root
  class={`rounded-[10px] border ${containerToneClass(tone)} ${compact ? "min-h-[8rem] px-4 py-5" : "min-h-[11rem] px-6 py-8"}`}
>
  <Empty.Header class="flex flex-col items-center text-center">
    <Empty.Media
      variant="icon"
      class={`mb-3 grid h-10 w-10 place-items-center rounded-[10px] text-sm ${mediaToneClass(tone)}`}
    >
      <i class={icon}></i>
    </Empty.Media>
    <Empty.Title class="m-0 text-base font-semibold text-foreground"
      >{title}</Empty.Title
    >
    {#if text}
      <Empty.Description
