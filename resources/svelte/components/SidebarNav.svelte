...
<nav
  class="flex h-full flex-col bg-sidebar text-sidebar-foreground"
  aria-label="Navigasi utama"
>
  <div class="border-b border-sidebar-border px-5 py-5">
    <a href={links.dashboard || "#"} class="block text-inherit no-underline">
      <div class="truncate text-base font-semibold text-sidebar-foreground">
        {appName}
      </div>
  ...
  <div class="flex-1 overflow-y-auto px-3 py-4">
    {#each navSections as section, index (section.title)}
      <section
        class={index > 0 ? "mt-6 border-t border-sidebar-border/70 pt-6" : ""}
      >
        <h2 class="px-2 pb-3 text-sm font-semibold text-sidebar-foreground">
          {section.title}
        </h2>
        ...
        <ul class="m-0 grid list-none gap-1.5 p-0">
          {#each section.items as item (item.href || item.label)}
            <li>
              ...
              >
                <span
                  class={`inline-flex w-5 shrink-0 items-center justify-center ${item.active ? "text-[color:var(--brand-hover)]" : "text-muted-foreground"}`}
                >
