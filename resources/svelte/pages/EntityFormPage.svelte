<script>
  ...
  const nativeSelectClass = (field) => `entity-select ${field.multiple ? 'entity-select-multiple' : ''}`;
</script>

<div class="mx-auto max-w-4xl">
    <Card.Root class="entity-form-card animate-fadeIn rounded-[10px] border border-border bg-card shadow-none">
      <Card.Header class="border-b border-border/70 pb-4">
        <PageHeader {title} {description} {icon} compact={true} />
      </Card.Header>

      <Card.Content class="pt-5">
        <form id={formId} method="POST" action={form.action} enctype={form.enctype || undefined}>
          ...
          <div class="entity-form-grid">
            {#each fields as field, index (field.name || index)}
              <div class={`entity-form-field ${field.span === 'half' ? 'entity-form-field-half' : ''} ${field.type === 'checkbox' ? 'entity-form-field-compact' : ''}`}>
                {#if field.type === 'checkbox'}
                  <label class={`entity-checkbox ${field.error ? 'is-invalid' : ''}`}>
                    <input
                      id={field.name}
                      name={field.name}
                      type="checkbox"
                  ...
                  {:else if field.type === 'select'}
                    ...
                    >
                      ...
                      {#each field.options || [] as option, optionIndex (option.value || optionIndex)}
                        <option value={option.value} selected={isSelected(field, option.value)}>{option.label}</option>
                      {/each}
                    </select>
                  {:else}
                    <Input
