<script>
  ...
  const contrastRatio = (foreground, background) => {
    const lighter = Math.max(
      srgbLuminance(foreground),
      srgbLuminance(background),
    );
  ...
  const ensureEventContrast = (eventElement) => {
    ...
    const softened = isDarkTheme
      ? blendWithBlack(background, 0.22)
      : blendWithWhite(background, 0.72);
    const border = isDarkTheme
      ? blendWithWhite(background, 0.12)
      : blendWithBlack(background, 0.08);
  ...
  const buildEventDetail = (event) => {
    ...
    return {
      ...
      url: event.url || "",
      accent:
        event.backgroundColor || event.borderColor || "var(--brand-secondary)",
      detailRows:
        props.type === "timeline"
          ? [
              ...
              ...(props.department
                ...
                : []),
              ...(props.program
                ? [{ label: "Program", value: props.program }]
                : []),
            ]
          : props.type === "program"
            ? [
  ...
  <Card.Content class="pt-5">
    <div class="timeline-calendar-legend">
      {#each legend as item, index (item.label || index)}
        <div
          class="timeline-calendar-legend-item"
          style={legendStyle(item.color)}
        >
          <span
            class="timeline-calendar-legend-dot"
    ...
    {:else}
      ...
      <div
        class={`timeline-calendar-surface ${isLoadingCalendar ? "timeline-calendar-surface-hidden" : ""}`}
...
{#if selectedEvent}
  <div class="timeline-modal-overlay">
    ...
    <div
      ...
      style={`--timeline-accent:${selectedEvent.accent};`}
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      <div class="timeline-modal-foot">
        <Button type="button" variant="secondary" onclick={closeEvent}
          >Tutup</Button
        >
...
<style>
  ...
  .timeline-calendar-legend-item {
    ...
    border: 1px solid
      color-mix(in srgb, var(--timeline-legend-color) 24%, var(--line-soft));
    border-radius: 0.625rem;
    background: color-mix(
      in srgb,
      var(--timeline-legend-color) 12%,
      var(--card)
    );
    color: color-mix(
      in srgb,
      var(--timeline-legend-color) 42%,
      var(--text-strong)
    );
    font-size: 0.88rem;
  }
  ...
  .timeline-modal-meta-item {
    ...
    background: color-mix(
      in srgb,
      var(--timeline-accent) 8%,
      var(--background)
    );
    border: 1px solid
      color-mix(in srgb, var(--timeline-accent) 18%, var(--line-soft));
  }
