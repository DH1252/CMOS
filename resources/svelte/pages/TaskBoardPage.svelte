<script>
  ...
  const hydrateBoard = (nextColumns) => {
    boardColumns = nextColumns.map((column) => {
      const previous = boardColumns.find(
  ...
  const [sendTask, receiveTask] = crossfade({
    duration: (distance) =>
      reduceMotion ? 0 : Math.min(260, Math.sqrt(distance) * 22 + 120),
    easing: quintOut,
  ...
  const findColumn = (status) =>
    boardColumns.find((column) => column.status === status);
  ...
  const replaceTask = (updatedTask) => {
    ...
    boardColumns = boardColumns.map((column) => {
      ...
      if (column.status === normalizedTask.status) {
        ...
        const insertIndex =
          currentLocation && currentLocation.status === normalizedTask.status
  ...
  const moveTaskPreview = (taskId, toStatus, toIndex) => {
    ...
    if (
      currentLocation.status === toStatus &&
      currentLocation.index === normalizedIndex
    ) {
      return;
    }
  ...
  const getDropIndex = (container, clientY) => {
    const cards = [
      ...container.querySelectorAll('[data-task-card="true"]'),
  ...
  const createDragPreview = (node, startX, startY, angle = -3) => {
    ...
    preview.style.borderColor =
      "color-mix(in srgb, var(--brand-primary) 46%, var(--line-soft))";
    preview.style.background =
      "color-mix(in srgb, var(--brand-light) 12%, var(--background))";
  ...
  const onDragStart = (event, taskId, status) => {
    ...
    if (event.dataTransfer) {
      ...
      const rect = card.getBoundingClientRect();
      const horizontalGrabRatio = Math.min(
        1,
      ...
      dragPreviewNode = createDragPreview(
        card,
        dragPreviewTargetX,
        dragPreviewTargetY,
        previewAngle,
...
>
  ...
  >
    ...
    <div class="grid min-w-[15rem] gap-3">
      <div class="rounded-[10px] border border-border bg-background px-4 py-4">
        ...
        <div class="mt-2 text-2xl leading-none font-semibold text-foreground">
          {totalTasks}
        </div>
      </div>
      <div
        class="grid gap-2 rounded-[10px] border border-border bg-background px-4 py-4"
      >
        {#each boardColumns as column, index (column.status || index)}
          <div class="flex items-center justify-between gap-3 text-sm">
            <span class="text-muted-foreground"
              >{column.label || statusLabels[column.status]}</span
            >
...
>
  {#each boardColumns as column, index (column.status || index)}
    <section
      class={`flex min-h-[32rem] w-[min(320px,88vw)] min-w-[min(320px,88vw)] shrink-0 snap-start flex-col rounded-[10px] border bg-card transition-colors duration-200 ${dragTargetStatus === column.status ? "kanban-column-target border-brand-primary/45 bg-brand-light/8" : "border-border"}`.trim()}
    >
      <div
        class={`flex items-center justify-between gap-3 border-b border-border p-4 pb-3 ${column.status === "todo" ? "" : ""}`}
      >
        <div class="flex items-center gap-2 text-sm font-bold">
          <span
            class={`h-3 w-3 rounded-full ${column.status === "todo" ? "bg-muted-foreground/30" : column.status === "in_progress" ? "bg-yellow-500" : column.status === "pending" ? "bg-[var(--brand-primary)]" : "bg-green-600"}`}
          ></span>
          <span>{column.label || statusLabels[column.status]}</span>
        </div>
        <StatusBadge
          label={String(column.tasks.length)}
          tone={columnTones[column.status] || "secondary"}
        />
      </div>
      ...
      >
          ...
          >
            <div class="flex items-start gap-2">
              ...
              <div class="flex shrink-0 items-center gap-2">
                <Button
                  href={task.showHref || taskUrl(task.id)}
                  variant="secondary"
                  size="sm"
                  class="px-2.5 shadow-none"
                >
                  <span>Buka</span>
                </Button>
                <DropdownMenu.Root>
            ...
            <div class="flex flex-wrap gap-2">
              <StatusBadge
                label={task.priority_label || priorityLabels[task.priority]}
            ...
            {#if task.progress > 0}
              <div class="flex items-center gap-2">
                <Progress value={task.progress} class="h-2 flex-1" />
                <span
                  class="min-w-[2.5rem] text-right text-xs font-bold text-muted-foreground"
                  >{task.progress}%</span
                >
              </div>
            {/if}
            ...
            <div
              class="flex flex-col justify-between gap-2 border-t border-border/70 pt-3 text-xs text-muted-foreground sm:flex-row sm:items-center"
            >
              <div class="flex items-center gap-2">
              ...
              {#if task.deadline_fmt}
                <div
                  class={`flex items-center gap-2 ${task.is_overdue ? "font-bold text-red-600" : ""}`.trim()}
                >
        ...
        {#if !column.tasks.length && !column.addOpen}
          <div
            class="grid min-h-[8rem] place-items-center gap-2 rounded-[10px] border border-dashed border-border p-4 text-center text-muted-foreground"
          >
        ...
        {#if column.addOpen}
          ...
          >
            ...
            <div class="grid gap-1.5">
              <Label for={`task-inline-description-${column.status}`}
                >Deskripsi</Label
              >
              <Textarea
            ...
            <div class="flex flex-col gap-3 sm:flex-row">
              <div class="grid flex-1 gap-1.5">
                <Label for={`task-inline-priority-${column.status}`}
                  >Prioritas</Label
                >
                <select
                  id={`task-inline-priority-${column.status}`}
                  class="h-10 w-full rounded-[10px] border border-border bg-background px-3 py-2 text-foreground focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 focus:outline-none"
                  onchange={(event) =>
                    updateDraft(
                      column.status,
                      "priority",
                ...
                >
                  <option
                    value="medium"
                    selected={column.draft.priority === "medium"}>Sedang</option
                  >
                  <option
                    value="high"
                    selected={column.draft.priority === "high"}>Tinggi</option
                  >
                  <option value="low" selected={column.draft.priority === "low"}
                    >Rendah</option
                  >
                </select>
              </div>
              ...
              <div class="grid flex-1 gap-1.5">
                <Label for={`task-inline-assignee-${column.status}`}
                  >Assignee</Label
                >
                <select
                ...
                >
                  ...
                  {#each users as user, userIndex (user.value || userIndex)}
                    <option
                      value={user.value}
                      selected={String(column.draft.assigned_to || "") ===
                        String(user.value)}>{user.label}</option
                    >
                  {/each}
                </select>
              </div>
            ...
            <div class="grid gap-1.5">
              <Label for={`task-inline-deadline-${column.status}`}
                >Deadline</Label
              >
              <Input
            ...
            <div class="mt-2 flex flex-col gap-2 sm:flex-row">
              <Button
                type="button"
                size="sm"
                onclick={() => void saveDraft(column.status)}
              >
              ...
              <Button
                ...
                variant="secondary"
                size="sm"
                onclick={() => resetDraft(column.status)}
              >
                <span>Batal</span>
              </Button>
            </div>
      ...
      <div class="mt-auto p-3">
        <Button
          type="button"
          variant="ghost"
          class="min-h-[2.8rem] w-full rounded-[10px] border border-dashed border-border font-semibold text-muted-foreground hover:border-border/80 hover:text-foreground"
          onclick={() => toggleAddForm(column.status, !column.addOpen)}
        >
          <i class="fas fa-plus"></i>
...
{#if editModalOpen && editingTask}
  ...
  >
    <button
      type="button"
      class="absolute inset-0 border-none bg-transparent"
    ...
    >
      ...
      <div class="grid gap-4 p-5">
        ...
        <div class="flex flex-col gap-4 sm:flex-row">
          <div class="grid flex-1 gap-1.5">
            ...
            <select
              id="edit-assignee"
              class="h-10 w-full rounded-[10px] border border-border bg-background px-3 py-2 text-foreground focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 focus:outline-none"
              bind:value={editingTask.assigned_to}
            >
              <option value="">Unassigned</option>
              {#each users as user, userIndex (user.value || userIndex)}
        ...
        <div class="grid gap-1.5 pt-2">
          ...
          <input
            ...
            max="100"
            step="5"
            class="mt-2 w-full accent-[var(--brand-primary)]"
      ...
      >
        ...
        <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
          ...
          >
            <i class={`fas ${savingEdit ? "fa-spinner fa-spin" : "fa-save"}`}
            ></i>
            <span>Simpan</span>
          </Button>
        </div>
