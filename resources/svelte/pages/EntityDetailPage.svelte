...
<div class="grid gap-4 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)]">
  <div>
    <Card.Root class="detail-summary-card animate-fadeIn rounded-[10px] border border-border bg-card shadow-none">
      <Card.Content class="space-y-5 pt-4">
        <div class="detail-summary-head">
          {#if summary.image}
            <img src={summary.image} alt={summary.title} class="avatar-xl" onerror={handleImageError} />
          {:else}
        ...
        {#if summary.actions?.length}
          <div class="detail-summary-actions">
            {#each summary.actions as item, itemIndex (item.href || item.label || itemIndex)}
              <Button href={item.href} variant={buttonVariant(item, itemIndex === 0)} class="detail-summary-action">
                {#if item.icon}
                  <i class={item.icon}></i>
                {/if}
  ...
  <div>
    ...
    {#each sections as section, sectionIndex (section.title || sectionIndex)}
      <Card.Root class={`animate-fadeIn rounded-[10px] border border-border bg-card shadow-none ${section.spacingClass || 'mb-4'}`.trim()}>
                ...
                {#each section.items as item, itemIndex (item.title || itemIndex)}
                  <div class="detail-list-item">
                    <div class="detail-list-item-main">
                      ...
                      {#if item.subtitle}
                        <div class="detail-list-item-subtitle">{item.subtitle}</div>
                      {/if}
                    </div>
                    {#if item.trailing}
                      <StatusBadge label={item.trailing.label} tone={item.trailing.tone || 'secondary'} icon={item.trailing.icon || ''} />
                    {/if}
                  </div>
                {/each}
              </div>
