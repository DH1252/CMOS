<script>
  ...
  const completionPercent = (department) => {
    ...
    return Math.round((safeNumber(department.evaluatedStaff) / department.totalStaff) * 100);
  };
        ...
        <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_15rem] lg:items-end">
          ...
          <form method="GET" action={monthAction} bind:this={monthForm} class="grid gap-2">
            ...
            <select
              ...
              name={monthParam}
              class="h-11 rounded-[10px] border border-border bg-background px-3 text-sm text-foreground outline-none transition-colors focus:border-brand-primary"
        ...
        {#if departments.length}
          <div class="grid gap-4 md:grid-cols-2">
            {#each departments as department, index (department.href || department.name || index)}
              <a href={department.href} class="rounded-[10px] border border-border bg-background px-5 py-5 text-inherit no-underline transition-colors hover:bg-muted">
                <div class="flex items-start justify-between gap-4">
                  <div class="min-w-0">
                    <h3 class="text-xl font-semibold leading-snug text-foreground">{department.name}</h3>
                    <p class="mt-2 text-sm leading-7 text-muted-foreground">{department.description}</p>
                  </div>
                  <div class="text-sm font-semibold text-brand-primary">{completionPercent(department)}%</div>
                </div>

                <div class="mt-4 h-2 overflow-hidden rounded-full bg-muted">
                  <div class="h-full rounded-full bg-brand-primary" style={`width:${completionPercent(department)}%`}></div>
                </div>

                <div class="mt-4 flex flex-wrap gap-2">
                  <StatusBadge label={`${department.totalStaff} staff`} icon="fas fa-user-group" tone="secondary" />
                  <StatusBadge label={`${department.evaluatedStaff} dinilai`} icon="fas fa-check-circle" tone="success" />
                </div>
              </a>
            {/each}
          </div>
        {:else}
          <EmptyStatePanel title={emptyDepartments.title} text={emptyDepartments.text} icon="fas fa-building" tone="info" compact={true} />
        {/if}
      </Card.Content>
    </Card.Root>
  </div>

  <Card.Root class="rounded-[10px] border border-border bg-card shadow-none xl:sticky xl:top-[5.5rem] xl:self-start">
    <Card.Header class="border-b border-border/70 pb-4">
      <PageHeader title="Best staff of the month" description={month?.label || ''} icon="fas fa-trophy" compact={true} headingTag="h3" />
    </Card.Header>
      ...
      {#if ranking.length}
        {#each ranking as item, index (item.name || index)}
          <div class="grid grid-cols-[auto_auto_minmax(0,1fr)_auto] items-center gap-3 rounded-[10px] border border-border bg-background px-4 py-3">
            ...
            <img src={item.avatar || fallbackAvatar(item.name)} alt={item.name} class="h-10 w-10 rounded-full object-cover" width="40" height="40" loading="lazy" decoding="async" onerror={handleImageError} />
            <div class="min-w-0">
              <strong class="block truncate text-sm text-foreground">{item.name}</strong>
              <span class="block truncate text-sm text-muted-foreground">{item.department}</span>
            </div>
            <div class="text-right">
              <div class="text-sm font-semibold text-brand-primary">{item.score}</div>
              {#if item.grade}
                <span class="mt-1 inline-flex rounded-[8px] bg-muted px-2 py-1 text-xs text-muted-foreground">{item.grade.label}</span>
              {/if}
            </div>
          </div>
        {/each}
      {:else}
        <EmptyStatePanel title={emptyRanking.title} text={emptyRanking.text} icon="fas fa-chart-line" tone="warning" compact={true} />
      {/if}
    </Card.Content>
  </Card.Root>
</div>
