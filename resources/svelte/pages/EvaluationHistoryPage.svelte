    ...
    {#if profile}
      ...
      >
        <Card.Content class="evaluation-profile-panel pt-5">
          <img
            src={profile.avatar || fallbackAvatar(profile.name)}
            alt={profile.name}
            class="avatar-xl evaluation-profile-avatar"
          ...
          {#if createAction}
            <Button
              ...
              variant={buttonVariant(createAction, true)}
              class="evaluation-profile-cta w-full"
            >
      ...
      <Card.Content class="evaluation-grade-stack pt-5">
        {#each gradeLegend as grade, index (grade.letter || index)}
          <div class="evaluation-grade-row">
            <span
              class="evaluation-grade-badge"
              style={`--grade-color:${grade.color};`}>{grade.letter}</span
            >
            <div>
    ...
    {#if periods.length}
      <div class="evaluation-period-list">
        {#each periods as period, index (period.label || index)}
          ...
          >
            <Card.Header
              class="evaluation-period-head border-b border-border/70 pb-4"
            >
              ...
              <div class="evaluation-period-head-meta">
                {#if period.status}
                  <StatusBadge
                    ...
                    tone={period.status.tone || "secondary"}
                  />
                {/if}
                {#if period.grade}
                  <span
                    class="evaluation-period-grade"
                    style={`--grade-color:${period.grade.color};`}
                  >
                    {period.grade.label}
                  </span>
                {/if}
                ...
                <div class="evaluation-summary-grid">
                  {#each period.summary as item, itemIndex (item.label || itemIndex)}
                    <div class="evaluation-summary-box">
                      ...
                      <strong class={item.muted ? "text-muted" : ""}
                        >{item.value}</strong
                      >
                      {#if item.badge}
                        <small
                          class="evaluation-inline-grade"
                          style={`--grade-color:${item.badge.color};`}
                          >{item.badge.label}</small
                        >
                      {/if}
                    </div>
                  {/each}
                  ...
                  <article class="evaluation-entry-card">
                    <div class="evaluation-entry-head">
                      ...
                      <div class="evaluation-entry-actions">
                        <strong
                          style={`color:${entry.scoreColor || "inherit"};`}
                          >{entry.score}</strong
                        >
                        {#if entry.editAction}
                          <Button
                          ...
                          >
                            <i class={entry.editAction.icon || "fas fa-pen"}
                            ></i>
                          </Button>
                        {/if}
                      </div>
                    ...
                    <div class="evaluation-criteria-mini-grid">
                      {#each entry.criteria as criterion, criterionIndex (criterion.label || criterionIndex)}
                        <div class="evaluation-mini-criterion">
                          ...
                          <div
                            class="evaluation-mini-dots"
                            aria-label={`${criterion.label}: ${criterion.value} dari 5`}
                          >
                            {#each [1, 2, 3, 4, 5] as score (score)}
                              <i
                                class={`fas fa-circle ${score <= criterion.value ? "evaluation-mini-dot-active" : "evaluation-mini-dot"}`}
                              ></i>
                            {/each}
                          </div>
                        </div>
...
<style>
  ...
  .evaluation-grade-badge {
    ...
    border: 1px solid
      color-mix(in srgb, var(--grade-color) 26%, var(--line-soft));
    font-weight: 800;
    flex-shrink: 0;
  }
  ...
  .evaluation-inline-grade {
    ...
    color: var(--text-strong);
    border: 1px solid
      color-mix(in srgb, var(--grade-color) 22%, var(--line-soft));
    font-size: 0.74rem;
    font-weight: 800;
  }
