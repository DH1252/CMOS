...
<div class="min-h-screen bg-background px-5 py-8 text-foreground lg:px-8 lg:py-10">
  <div class="mx-auto max-w-[720px]">
    <Card.Root class="rounded-[10px] border border-border bg-card shadow-none">
      <Card.Header class="border-b border-border pb-4">
        <a href={homeUrl} data-native="true" class="mb-4 inline-flex items-center gap-2 rounded-[10px] border border-border bg-background px-3.5 py-2 text-sm font-medium text-foreground no-underline transition-colors hover:border-brand-primary hover:bg-muted/70" aria-label="Kembali ke beranda">
      ...
      <Card.Content class="grid gap-5 pt-5">
        {#if alertMessage}
          <div class={`rounded-[10px] border px-4 py-3 text-sm ${alertType === 'info' ? 'border-[color:color-mix(in_srgb,var(--signal-info)_30%,transparent)] bg-[color:color-mix(in_srgb,var(--signal-info)_12%,transparent)] text-[var(--signal-info)]' : 'border-[color:color-mix(in_srgb,var(--signal-danger)_30%,transparent)] bg-[color:color-mix(in_srgb,var(--signal-danger)_12%,transparent)] text-[var(--signal-danger)]'}`} role="alert">
            {alertMessage}
          </div>
        {/if}
        ...
        <form method="POST" action={loginUrl} class="grid gap-5">
          ...
          <div class="grid gap-4 rounded-[10px] border border-border bg-background px-4 py-4">
          ...
          <div class="flex flex-col gap-4 rounded-[10px] border border-border bg-background px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
